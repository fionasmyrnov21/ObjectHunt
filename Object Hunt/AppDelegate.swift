...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func clearTemporaryForceNativeIfNeeded() {
        ...
        UserDefaults.standard.removeObject(forKey: "permanent_native_flow")
        UserDefaults.standard.removeObject(forKey: "saved_portal_destination")
        UserDefaults.standard.removeObject(forKey: "saved_portal_destination_expires")
    ...
    private func scheduleLaunchLoaderFinish() {
        let workItem = DispatchWorkItem { [weak self] in
            ...
            if !self.didReceiveConfigResponse
                ...
                if LaunchState.shared.activateStoredDestinationIfValid() == false {
    ...
    private func continueLaunchFlow() {
        if LaunchState.shared.didOpenPushDestination {
        ...
        if LaunchState.shared.activateStoredDestinationIfValid() {
            OrientationController.shared.unlockAllOrientations()
            if let storedPayload = LaunchState.shared.storedConfigPayload() {
                let apply = !LaunchState.shared.didOpenPushDestination
                sendMergedPayload(storedPayload, applyDestinationToCurrentSession: apply)
            } else if LaunchState.shared.isStoredDestinationExpired() {
