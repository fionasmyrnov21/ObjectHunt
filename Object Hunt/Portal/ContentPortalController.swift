...
final class ContentPortalController: UIViewController {
    ...
    private func updateErrorBackground(for size: CGSize) {
        ...
        imageView.image = UIImage(named: backgroundImageName(isLandscape: size.width > size.height, noInternet: errorUsesNoInternetArt))
    }
    ...
    func handleMainFrameLoadFailure(_ error: NSError) {
        ...
        showErrorView(message: errorMessage(for: error), noInternet: isOfflineError(error))
    }
    ...
    private func showErrorView(message: String, noInternet: Bool) {
        finishInitialLoading()
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            self.errorUsesNoInternetArt = noInternet
            let isLandscape = self.view.bounds.width > self.view.bounds.height
            let imageName = self.backgroundImageName(isLandscape: isLandscape, noInternet: noInternet)

            if let existing = self.errorView {
                self.errorMessageLabel?.text = message
                self.errorMessageLabel?.isHidden = noInternet
                self.errorBackgroundImageView?.image = UIImage(named: imageName)
                if let retry = existing.viewWithTag(902) {
            ...
            NSLayoutConstraint.activate([
                ...
                retryButton.bottomAnchor.constraint(equalTo: container.safeAreaLayoutGuide.bottomAnchor, constant: -28)
            ])
